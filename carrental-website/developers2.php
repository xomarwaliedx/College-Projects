...
function filter($db, $tableName, $columns)
{
   if (empty($db)) {
      ...
      $columnName = implode(", ", $columns);
      $query = "SELECT " . $columnName . " FROM`office` NATURAL JOIN `car` NATURAL JOIN `specs` WHERE `model` LIKE '%" . $_POST["models"] . "%' AND `milage` BETWEEN " . $_POST["miles"] . " AND " . $_POST["miles2"] . " AND `color` LIKE '%" . $_POST["colors"] . "%' AND `officeloc` LIKE '%" . $_POST["locs"] . "%' AND `year` BETWEEN " . $_POST["years"] . " AND " . $_POST["years2"] . " AND `cc` BETWEEN " . $_POST["ccs"] . " AND " . $_POST["ccs2"] . " ORDER BY model DESC";
      // echo $query;
      $result = $db->query($query);
      if ($result == true) {
         if ($result->num_rows > 0) {
